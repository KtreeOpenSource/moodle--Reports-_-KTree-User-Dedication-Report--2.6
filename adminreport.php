...
<?php
	...
	while ($countervalue<count($allusers)) {
		...
		$name = $fname.' '.$lname;
        $logintime = $DB->get_record_sql("SELECT * FROM `{log}` where userid='".$allusers[$countervalue]."' and action='login' and (time  >='". $starttime ."' and time  <='". $maxtime ."') order by id limit 0,1");
        if ($logintime == null) {
            $firstaccess = '-';
        } else {
            $firstaccess = date('m-d-Y h:i:s a',$logintime->time);
        }
        $lastlogintime = $DB->get_record_sql("SELECT * FROM `{log}` where userid='".$allusers[$countervalue]."' and action='login' and (time  >='". $starttime ."' and time  <='". $maxtime ."') order by id desc limit 0,1");			
        if ($lastlogintime == null) {
            $lastlogin = '-';
        } else {
            $lastlogin = date('m-d-Y h:i:s a',$lastlogintime->time);
        }
        $logouttime = $DB->get_record_sql("SELECT * FROM `{log}` where userid='".$allusers[$countervalue]."' and action='logout' and (time  >='". $starttime ."' and time  <='". $maxtime ."') order by id desc limit 0,1");
        if ($logouttime == null) {
        $lastaccess = '-';
        } else {
        $lastaccess = date('m-d-Y h:i:s a',$logouttime->time);
        }
?>
<tr>
